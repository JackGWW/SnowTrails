  ...
  onRegionDidChange = async () => {
    if (this.mapView) {
      try {
        ...
        // Convert zoom to longitudeDelta approximation for backward compatibility
        // longitudeDelta ≈ 360 / (2^zoom * mapWidth)
        const mapWidth = Dimensions.get('window').width;
        const longitudeDelta = 360 / (Math.pow(2, zoom) * mapWidth / 256);
